  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6">
          {/* Header */}
          <h1 className="text-3xl font-bold text-gray-800 mb-6">Ã¢Å¡â„¢Ã¯Â¸Â ConfiguraÃƒÂ§ÃƒÂµes da InstituiÃƒÂ§ÃƒÂ£o</h1>
          ...
          <div className="flex border-b border-gray-300 bg-white rounded-t-lg overflow-x-auto mb-6">
            ...
            >
              Ã°Å¸Ââ€ºÃ¯Â¸Â Perfil da InstituiÃƒÂ§ÃƒÂ£o
            </button>
            <button
            ...
            >
              Ã¢Å¡â€“Ã¯Â¸Â Cargos Ministeriais
            </button>
          </div>
...
function PerfilContent({ onNotification }: { onNotification: (title: string, message: string, type: 'success' | 'error' | 'warning' | 'info') => void }) {
  ...
  useEffect(() => {
    fetchConfiguracaoIgrejaFromSupabase(supabase)
      .then((config: any) => {
        setFormData({
          nomeMinisterio: config.nome || 'InstituiÃƒÂ§ÃƒÂ£o',
          cnpj: config.cnpj || '',
          email: config.email || '',
  ...
  const handleSave = async () => {
    try {
      ...
      onNotification('Sucesso', 'Dados da instituiÃƒÂ§ÃƒÂ£o atualizados com sucesso!', 'success');
      setIsEditing(false);
    } catch (error: any) {
      console.error('Ã¢ÂÅ’ Erro ao salvar perfil:', error);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Perfil da InstituiÃƒÂ§ÃƒÂ£o</h2>
        <button
        ...
        >
          {isEditing ? 'Ã¢ÂÅ’ Cancelar' : 'Ã¢Å“ÂÃ¯Â¸Â Editar'}
        </button>
      </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Nome da InstituiÃƒÂ§ÃƒÂ£o</label>
            <input
              type="text"
              name="nomeMinisterio"
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">EndereÃƒÂ§o</label>
          <textarea
            name="endereco"
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">DescriÃƒÂ§ÃƒÂ£o</label>
          <textarea
            name="descricao"
            ...
            disabled={!isEditing}
            rows={3}
            placeholder="InformaÃƒÂ§ÃƒÂµes sobre sua instituiÃƒÂ§ÃƒÂ£o"
            className="w-full px-4 py-2 border border-gray-300 rounded-lg disabled:bg-gray-100 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-teal-500"
          />
        </div>

        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">Data de FundaÃƒÂ§ÃƒÂ£o</label>
          <input
            type="date"
            name="dataCadastro"
        ...
        {isEditing && (
          <div className="flex gap-4 pt-4">
            ...
            >
              Ã¢Å“â€œ Salvar AlteraÃƒÂ§ÃƒÂµes
            </button>
            <button
...
function BrandingContent({ onNotification }: { onNotification: (title: string, message: string, type: 'success' | 'error' | 'warning' | 'info') => void }) {
  ...
  useEffect(() => {
    supabase
      ...
      .maybeSingle()
      .then(({ data }: { data: any }) => {
        const logo = (data?.value as any)?.base64 || null;
        setLogoPreview(logo);
      });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          if (img.width < 200 || img.height < 200) {
            onNotification('Aviso', 'A imagem deve ter no mÃƒÂ­nimo 200x200 pixels', 'warning');
            return;
          }
  ...
  const handleSaveLogo = async () => {
    ...
    try {
      const { error } = await supabase
        .from('configurations')
        .upsert({ key: LOGO_KEY, value: { base64: logoPreview } }, { onConflict: 'key' });
      if (error) throw error;
      onNotification('Sucesso', 'Logomarca salva com sucesso!', 'success');
    } catch (err: any) {
      console.error('Erro ao salvar logo:', err);
      onNotification('Erro', `NÃƒÂ£o foi possÃƒÂ­vel salvar a logomarca: ${err?.message || 'tente novamente'}`, 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center bg-gray-50 mb-4">
            ...
            <p className="text-gray-500 text-xs mb-4">DimensÃƒÂµes recomendadas: 500x500px<br />Formatos: PNG, JPG, SVG | MÃƒÂ¡ximo: 5MB</p>
            <input
              type="file"
              accept="image/*"
            ...
            >
              Ã°Å¸â€œÂ Escolher Imagem
            </label>
          </div>
        </div>

        <div>
          <h3 className="text-lg font-bold text-gray-800 mb-4">PrÃƒÂ©via da Logomarca</h3>
          ...
          <div className="border border-gray-300 rounded-lg p-8 text-center bg-gray-50 mb-4 h-64 flex items-center justify-center">
            {logoPreview ? (
              ...
              <div className="text-gray-400">
                <div className="text-5xl mb-2">Ã°Å¸â€“Â¼Ã¯Â¸Â</div>
          ...
          {logoPreview && (
            <button
              ...
              disabled={saving}
              className="w-full px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {saving ? 'Ã¢ÂÂ³ Salvando...' : 'Ã¢Å“â€œ Salvar Logomarca'}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-sm text-gray-700">
          Ã°Å¸â€™Â¡ <strong>Dica:</strong> As informaÃƒÂ§ÃƒÂµes da chiesa (nome, endereÃƒÂ§o, CNPJ, telefone, email) sÃƒÂ£o configuradas na aba <strong>"Perfil da InstituiÃƒÂ§ÃƒÂ£o"</strong> e serÃƒÂ£o exibidas automaticamente no cabeÃƒÂ§alho dos relatÃƒÂ³rios em PDF.
        </p>
      </div>
    </div>
  );
...
function NomenclaturaContent({ onNotification }: { onNotification: (title: string, message: string, type: 'success' | 'error' | 'warning' | 'info') => void }) {
  ...
  const NATIVE_OPTIONS: Record<DivisionKey, string[]> = {
    // DivisÃƒÂ£o 1 agora usa as opÃƒÂ§ÃƒÂµes que eram da DivisÃƒÂ£o 3
    divisaoPrincipal: ['CONGREGAÃƒâ€¡ÃƒÆ’O', 'IGREJA', 'TEMPLO', 'NENHUMA'],
    divisaoSecundaria: ['CAMPO', 'SETOR', 'GRUPO', 'ÃƒÂREA', 'NENHUMA'],
    // DivisÃƒÂ£o 3: deixar apenas "NENHUMA" (usuÃƒÂ¡rio pode adicionar manualmente)
  ...
  const handleAddCustomOption = (nivel: DivisionKey) => {
    ...
    if (exists) {
      onNotification('Aviso', 'Essa opÃƒÂ§ÃƒÂ£o jÃƒÂ¡ existe.', 'warning');
      return;
    }
  ...
  const handleSave = async () => {
    try {
      ...
      console.error('Ã¢ÂÅ’ Erro ao salvar cargos:', error);
      onNotification('Erro', `Erro ao salvar: ${error?.message || 'Tente novamente'}`, 'error');
    }
  };

  const handleCancel = () => {
    console.log('Ã¢ÂÅ’ Cancelando ediÃƒÂ§ÃƒÂ£o, revertendo para:', nomenclaturas);
  ...
  const adicionarCargo = () => {
    ...
    if (jaExiste) {
      onNotification('Aviso', 'Este cargo jÃƒÂ¡ existe na lista.', 'warning');
      return;
    }
  ...
  return (
    <div>
      ...
      {/* Placeholder para manter estrutura - seÃƒÂ§ÃƒÂ£o removida */}
      {false && <div className="mb-8">
        <h3 className="text-xl font-bold text-gray-800 mb-4">Ã°Å¸ÂÂ¢ DivisÃƒÂµes Organizacionais</h3>
          ...
          {(['divisaoPrincipal', 'divisaoSecundaria', 'divisaoTerciaria'] as DivisionKey[]).map((key, index) => (
            <div key={key} className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
              <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <span>{index + 1}Ã¯Â¸ÂÃ¢Æ’Â£</span> {index === 0 ? 'Primeira' : index === 1 ? 'Segunda' : 'Terceira'} DivisÃƒÂ£o
              </h3>
                    ...
                    <div className="space-y-3">
                      ...
                      {(temp[key].custom || []).length > 0 && (
                        <div className="space-y-2">
                          <p className="text-xs text-gray-600">OpÃƒÂ§ÃƒÂµes personalizadas:</p>
                          <div className="space-y-2">
                            {(temp[key].custom || []).map(option => (
                              <div key={option} className="flex items-center justify-between p-2 border border-gray-200 rounded-lg bg-gray-50">
                                ...
                                <button
                                  ...
                                  title="Remover opÃƒÂ§ÃƒÂ£o personalizada"
                                >
                                  Ã°Å¸â€”â€˜Ã¯Â¸Â
                                </button>
                              </div>
      ...
      {/* SeÃƒÂ§ÃƒÂ£o: Cargos Ministeriais */}
      <div className="mb-8">
        <h3 className="text-xl font-bold text-gray-800 mb-4">Ã¢â€ºÂª Cargos Ministeriais</h3>
        <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
          ...
          {isEditing && (
            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <h4 className="text-sm font-bold text-gray-800 mb-3">Ã¢Å¾â€¢ Adicionar Novo Cargo</h4>
              <div className="flex gap-3">
                <input
                  type="text"
                  value={novoCargo}
              ...
              <p className="text-xs text-gray-600 mt-2">
                Ã°Å¸â€™Â¡ Digite o nome do novo cargo e clique em "Adicionar". VocÃƒÂª pode remover cargos personalizados clicando no ÃƒÂ­cone Ã°Å¸â€”â€˜Ã¯Â¸Â.
              </p>
            </div>
          )}

          <p className="text-xs text-gray-600 mt-4">
            Ã°Å¸â€™Â¡ Marque os cargos que deseja disponibilizar no sistema. Eles aparecerÃƒÂ£o no formulÃƒÂ¡rio de cadastro de ministros.
          </p>
        </div>
      </div>
