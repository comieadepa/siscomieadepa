...
import { History, ChevronLeft, ChevronRight, RefreshCw, Search, X, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function FinanceiroHistoricoPage() {
  ...
  const carregarLogs = useCallback(async () => {
    try {
      ...
      let query = supabase
        .from('audit_logs')
        .select('id, usuario_email, acao, modulo, area, descricao, status, created_at, tabela_afetada, dados_anteriores, dados_novos, mensagem_erro')
  ...
  }, [supabase, filtroAcao, filtroStatus, filtroPeriodo, filtroUsuario]);
  ...
  useEffect(() => {
    ...
    void registrarAcao({
      acao: 'visualizar_historico',
      modulo: 'financeiro',
      descricao: 'Visualizou histórico financeiro',
    });
  }, [authLoading, roleLoading, podeAcessar, registrarAcao]);

  // Filtro local por descrição
  ...
  return (
    <PageLayout title="Financeiro" description="Histórico de ações financeiras" activeMenu="financeiro">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <History size={20} className="text-[#123b63]" />
              <h1 className="text-2xl font-bold text-[#123b63]">Histórico Financeiro</h1>
            </div>
            <p className="text-sm text-gray-500 mt-0.5">Registro de todas as ações realizadas no módulo financeiro</p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <div>
              ...
              <input type="text" placeholder="E-mail do usuário" value={filtroUsuario}
                onChange={e => setFiltroUsuario(e.target.value)}
                className="border border-gray-300 rounded-lg px-3 py-1.5 text-xs text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-300 w-48" />
            </div>
            <div className="flex-1 min-w-48">
              <label className="text-xs font-bold text-gray-500 uppercase block mb-1">Busca</label>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-gray-50">
              {paginated.map((log) => {
                ...
                const temDetalhes = log.dados_anteriores || log.dados_novos || log.mensagem_erro;
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-3 mt-0.5">
                          <span className="text-[10px] text-gray-400">{log.usuario_email ?? 'sistema'}</span>
                          {log.tabela_afetada && (
                            <span className="text-[10px] text-gray-400">tabela: {log.tabela_afetada}</span>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className="text-[10px] text-gray-400">{fmtData(log.created_at)}</span>
                        {temDetalhes && (
                          isExpanded
                    ...
                    {isExpanded && temDetalhes && (
                      <div className="px-5 pb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        {log.dados_anteriores && (
                          <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Dados Anteriores</p>
                            <pre className="bg-gray-50 rounded-lg p-3 text-[10px] text-gray-600 overflow-auto max-h-40 border border-gray-100">
                        ...
                        {log.dados_novos && (
                          <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Dados Novos</p>
                            <pre className="bg-emerald-50 rounded-lg p-3 text-[10px] text-emerald-800 overflow-auto max-h-40 border border-emerald-100">
                        ...
                        {log.mensagem_erro && (
                          <div className="md:col-span-2">
                            <p className="text-[10px] font-bold text-red-400 uppercase mb-1">Mensagem de Erro</p>
                            <p className="bg-red-50 rounded-lg p-3 text-[10px] text-red-700 border border-red-100 font-mono">{log.mensagem_erro}</p>
                          </div>
                        )}
                      </div>
                    )}
