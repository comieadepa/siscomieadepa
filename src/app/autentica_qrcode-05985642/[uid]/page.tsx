...
export default function CredencialDigitalPage({ params }: { params: Promise<{ uid: string }> }) {
  ...
  /** Formata data ISO para pt-BR. Retorna '—' se ausente, inválida ou no futuro (para nascimento). */
  const fmtDate = (v?: string, opts?: { rejectFuture?: boolean }) => {
    if (!v || v === 'null' || v === 'undefined') return '—';
    const d = new Date(v + (v.length === 10 ? 'T12:00:00' : ''));
    if (isNaN(d.getTime())) return '—';
    if (opts?.rejectFuture && d > new Date()) return '—';
    ...
    <div style={{
      ...
      {!loading && dados && (
        <>
          ...
          {mostrando === 'verso' && (
            <div style={{
              ...
              <div style={{ position: 'absolute', inset: 0 }}>
                ...
                {/* DATA DE NASC. — rejeita datas no futuro (erro de digitação) */}
                <div style={{
          ...
          >
            🔄 {mostrando === 'frente' ? 'Ver verso' : 'Ver frente'}
          </button>
