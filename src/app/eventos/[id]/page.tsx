...
function TabInscritos({ inscricoes, loading, supervisoes, campos, nomeSup, nomeCampo, onRefresh, supabase, evento, podeEditar, podeRemover, podeMover, podeComunicacao, podeCertificados }: {
  ...
  async function carregarDadosOperacionais(inscricaoId: string, eventoIdFonte: string) {
    ...
    try {
      ...
      try {
        const [hospRes, leitosRes] = await Promise.all([
          fetch(`/api/eventos/${eventoIdFonte}/hospedagens`),
          fetch(`/api/eventos/${eventoIdFonte}/hospedagens/leitos`),
        ]);
        if (hospRes.ok) {
  ...
  function abrirEdicao(ins: Inscricao) {
    ...
    setErroEdit(null);
    setEditando(ins);
    carregarDadosOperacionais(ins.id, ins.evento_id);
    setEditForm({
