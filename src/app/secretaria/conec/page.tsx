...
export default function ConecDashboardPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Deseja realmente remover a instituição "${name}"?`)) {
      return;
    }
    try {
  ...
  return (
    <PageLayout
      title="Painel CONEC"
      description="Conselho de Educação Cristã — Gestão Credenciamento de Instituições Teológicas"
      activeMenu="conec"
    >
      <div className="space-y-6">
        {/* METRICS CARDS */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-sm flex items-center gap-5">
            <div className="w-12 h-12 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0">
              <Building2 className="w-6 h-6" />
            </div>
            <div>
              <p className="text-gray-500 text-sm font-medium">Total de Instituições</p>
          ...
          <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-sm flex items-center gap-5">
            ...
            <div>
              <p className="text-gray-500 text-sm font-medium">Instituições Ativas</p>
          ...
          <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-sm flex items-center gap-5">
            ...
            <div>
              <p className="text-gray-500 text-sm font-medium">Instituições Inativas</p>
        ...
        {/* CONTROLS */}
        <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex flex-1 flex-col md:flex-row gap-3">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
              />
            </div>
            <select
              value={statusFilter}
              onChange={(e: any) => setStatusFilter(e.target.value)}
              className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
            >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 bg-[#0D2B4E] hover:bg-[#153e6d] text-white font-semibold px-5 py-2.5 rounded-xl transition-all shadow-sm text-sm"
          >
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
                  {filteredInstituicoes.map((inst) => (
                    <tr key={inst.id} className="hover:bg-gray-50/55 transition-colors">
