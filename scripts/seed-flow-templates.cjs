/* eslint-disable no-console */
const { createClient } = require('@supabase/supabase-js');

const supabaseUrl = process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL;
const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

if (!supabaseUrl || !serviceKey) {
  console.error('Missing env: SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY');
  process.exit(1);
}

const adminClient = createClient(supabaseUrl, serviceKey, {
  auth: { autoRefreshToken: false, persistSession: false },
});

const seeds = [
  {
    name: 'Batismo',
    description: 'Fluxo padrao para batismo.',
    definition: {
      initial_status: 'pendente',
      statuses: ['pendente', 'em_analise', 'aprovado', 'rejeitado', 'concluido'],
      final_statuses: ['concluido', 'rejeitado'],
      initial_assignee_role: 'OPERADOR',
      transitions: [
        { action: 'iniciar_analise', from: 'pendente', to: 'em_analise', roles: ['OPERADOR', 'SUPERVISOR'] },
        { action: 'aprovar', from: 'em_analise', to: 'aprovado', roles: ['SUPERVISOR', 'ADMINISTRADOR'] },
        { action: 'rejeitar', from: 'em_analise', to: 'rejeitado', roles: ['SUPERVISOR', 'ADMINISTRADOR'], require_note: true },
        { action: 'concluir', from: 'aprovado', to: 'concluido', roles: ['OPERADOR', 'ADMINISTRADOR'] },
      ],
    },
  },
  {
    name: 'Apresentacao de Criancas',
    description: 'Fluxo padrao para apresentacao de criancas.',
    definition: {
      initial_status: 'pendente',
      statuses: ['pendente', 'em_analise', 'aprovado', 'rejeitado', 'concluido'],
      final_statuses: ['concluido', 'rejeitado'],
      initial_assignee_role: 'OPERADOR',
      form: {
        fields: [
          { name: 'crianca_nome', label: 'Nome da crianca', type: 'text', required: true },
          { name: 'child_id', label: 'ID da crianca (se ja cadastrada)', type: 'text', required: false },
          { name: 'responsavel_1', label: 'Responsavel 1', type: 'text', required: true },
          { name: 'responsavel_2', label: 'Responsavel 2', type: 'text', required: false },
          { name: 'data_apresentacao', label: 'Data da apresentacao', type: 'date', required: true },
          { name: 'culto_turno', label: 'Culto/Turno', type: 'select', required: false, options: ['Manha', 'Tarde', 'Noite'] },
          { name: 'observacoes', label: 'Observacoes', type: 'textarea', required: false },
        ],
      },
      transitions: [
        { action: 'iniciar_analise', label: 'Enviar para analise', from: 'pendente', to: 'em_analise', roles: ['OPERADOR', 'SUPERVISOR'] },
        { action: 'aprovar', label: 'Aprovar', from: 'em_analise', to: 'aprovado', roles: ['SUPERVISOR', 'ADMINISTRADOR'] },
        { action: 'rejeitar', label: 'Rejeitar', from: 'em_analise', to: 'rejeitado', roles: ['SUPERVISOR', 'ADMINISTRADOR'], require_note: true },
        { action: 'concluir', label: 'Concluir', from: 'aprovado', to: 'concluido', roles: ['OPERADOR', 'ADMINISTRADOR'] },
      ],
    },
  },
];

async function seedForMinistry(ministryId) {
  const { data: existing } = await adminClient
    .from('flow_templates')
    .select('id')
    .eq('ministry_id', ministryId)
    .limit(1);

  if (existing && existing.length > 0) return { created: 0 };

  let created = 0;
  for (const seed of seeds) {
    const { data: template, error } = await adminClient
      .from('flow_templates')
      .insert({
        ministry_id: ministryId,
        name: seed.name,
        description: seed.description,
        current_version: 1,
        is_published: true,
      })
      .select('id')
      .single();

    if (error || !template) continue;

    await adminClient
      .from('flow_template_versions')
      .insert({
        template_id: template.id,
        version: 1,
        definition_json: seed.definition,
        published_at: new Date().toISOString(),
      });

    created += 1;
  }

  return { created };
}

async function main() {
  const { data: ministries, error } = await adminClient
    .from('ministries')
    .select('id');

  if (error) throw error;

  let total = 0;
  for (const ministry of ministries || []) {
    const result = await seedForMinistry(ministry.id);
    total += result.created;
  }

  console.log('Seed concluido:', { total_created: total, ministries: (ministries || []).length });
}

main().catch(err => {
  console.error('Seed falhou:', err?.message || err);
  process.exit(1);
});
